...
describe("cline free-models envelope in nonStreamingHandler", () => {
  ...
  it("leaves an enveloped body untouched for a provider that did not opt in", async () => {
    // KRouter9 note: the nonStreamingHandler path additionally applies the V7
    // "200 trap" (an OpenAI-format body with no `choices` is rejected), so this
    // invariant is asserted on the unwrap helper directly instead of the handler.
    const { unwrapClineEnvelope } = await import("../../open-sse/shared/clineEnvelope.js");
    const enveloped = {
      success: true,
      data: { choices: [{ message: { content: "Hi" } }] },
    };
    const out = unwrapClineEnvelope(enveloped, "openai");
    expect(out.success).toBe(true);
    expect(out.data.choices[0].message.content).toBe("Hi");
