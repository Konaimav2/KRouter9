...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.krouter9/runtime
...
// Identifiers for killAllAppProcesses - only kill krouter9 specifically
const PROCESS_IDENTIFIERS = [
  'krouter9'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "krouter9")
    : path.join(os.homedir(), ".krouter9");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running krouter9/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "krouter9" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("krouter9") && (cmd.includes("cli.js") || cmd.includes("\\krouter9") || cmd.includes("/krouter9")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running krouter9/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "krouter9".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("krouter9") && (cmd.includes("cli.js") || cmd.includes("/krouter9")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 KRouter9 is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 KRouter9 is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "krouter9", "db.json") : path.join(".krouter9", "db.json"));
