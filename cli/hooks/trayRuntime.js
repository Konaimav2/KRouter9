...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.krouter9/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/krouter9/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[krouter9][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[krouter9][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[krouter9][runtime] chmod tray bin failed: ${e.message}`);
  }
}
