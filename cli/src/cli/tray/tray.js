...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `KRouter9 (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[krouter9] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[krouter9] tray init error: ${err.message}\n`);
    return null;
  }
}
