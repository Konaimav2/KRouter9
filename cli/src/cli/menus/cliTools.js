...
async function buildCodexHeader() {
  ...
  const { installed, hasKRouter9, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasKRouter9, settings } = result.data;
  ...
  // Extract KRouter9 custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:KRouter9-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract KRouter9 provider config
  const provider = settings?.models?.providers?.["krouter9"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("krouter9/") ? primary.replace("krouter9/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasKRouter9, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use KRouter9\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
