...
import { getCodeBuddyAccount, deleteCodeBuddyAccount, markCodeBuddyRunning, markCodeBuddySuccess, markCodeBuddyError, markCanvaEnrolled, createCodeBuddyJob, updateCodeBuddyJobResult, updateCodeBuddyJobStatus } from "@/lib/db/index.js";
...
export async function POST(req, ctx) {
  ...
  const body = {};
  try { Object.assign(body, await req.json()); } catch {}
  const res = {
    _status: 200,
    status(code) { this._status = code; return this; },
    json(obj) { return new Response(JSON.stringify(obj), { status: this._status, headers: { "Content-Type": "application/json" } }); },
  };
  return POST_handler({ body }, res, { params });
}
export async function POST_handler(req, res, { params }) {
    try {
        ...
        const body = req.body;
        const { action, deleteFromkrouter9 } = body;
        ...
        // ── Action: Inject API key to krouter9 ───────────────────────────
        if (action === "add-to-krouter9") {
            ...
            try {
                ...
                return res.json({ ok: true, email: account.email, message: `✓ ${account.email} was added to the ${provider} provider in KRouter9.` });
            }
            catch (e) {
                return res.status(500).json({ error: `Failed to add the account to KRouter9: ${e.message}` });
            }
        }
        // ── Action: Delete account ───────────────────────────────────────
        if (action === "delete") {
            if (deleteFromkrouter9 && account.email && account.provider) {
