...
import {listCodeBuddyAccounts, getCodeBuddyAccount, insertCodeBuddyAccount, bulkDeleteCodeBuddyAccounts, deleteCodeBuddyAccount, markCodeBuddyRunning, markCodeBuddySuccess, markCodeBuddyError, markCanvaEnrolled, createCodeBuddyJob, getCodeBuddyJob, updateCodeBuddyJobStatus, updateCodeBuddyJobResult} from "@/lib/db/index.js";

// KRouter9 local helpers (provider connection by email+provider)
async function deleteProviderConnectionByEmailAndProvider(email, provider) {
  const db = await getAdapter();
  db.run("DELETE FROM providerConnections WHERE provider = ? AND email = ?", [provider, email]);
}
async function createProviderConnection(data) {
  const db = await getAdapter();
  const now = new Date().toISOString();
  const id = crypto.randomUUID ? crypto.randomUUID() : crypto.randomBytes(16).toString("hex");
  db.run("INSERT INTO providerConnections (id, provider, authType, name, email, data, isActive, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [id, data.provider || "codebuddy", data.authType || "apikey", data.name || email, data.email || "", JSON.stringify(data.data || {}), 1, now, now]);
  return id;
}
...
function generateHumanAlias() {
    ...
    if (style === 0)
        return `${first}${digits(dLen)}`;
    if (style === 1)
        return `${first}.${digits(dLen)}`;
    if (style === 2)
        return `${first}_${digits(dLen)}`;
...
export async function GET(req) {
  const res = {
    ...
    json(obj) {
      return new Response(JSON.stringify(obj), { status: this._status, headers: { "Content-Type": "application/json" } });
    },
  };
    try {
        ...
        return res.json({
            ...
            settings: {
                auto_krouter9: settings.codebuddy_auto_krouter9 || "0",
...
export async function POST_handler(req, res) {
    try {
        ...
        // ── Action: Settings ─────────────────────────────────────────────
        if (action === "settings") {
            const { auto_krouter9, browser_headless, debug_mode, leave_canva_team, proxy_enabled, proxy_pool, leonardo_invite_link, codebuddy_2captcha_api_key } = body;
            const updates = {};
            if (auto_krouter9 !== undefined)
                updates.codebuddy_auto_krouter9 = auto_krouter9 ? "1" : "0";
        ...
        if (action === "bulk-delete") {
            const { statuses, provider, deleteFromkrouter9 } = body;
            ...
            for (const acc of deleted) {
                if (deleteFromkrouter9 && acc.email && acc.provider) {
        ...
        if (action === "bulk-delete-ids") {
            const { ids, provider, deleteFromkrouter9 } = body;
        ...
        // ── Action: Bulk add ready accounts to krouter9 ─────────────────
        if (action === "bulk-add-to-krouter9") {
            const { provider } = body;
            const accounts = await listCodeBuddyAccounts();
            ...
            return res.json({
                ...
                failed,
                errors,
                message: `✓ ${success}/${ready.length} accounts were added to KRouter9.`
            });
        }
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        if (parsed.step) {
                            ...
                            if (settings.codebuddy_auto_krouter9 === "1" || isLeonardo || isWeavy || isKimi || isQoder || isCloudflare) {
                                try {
                                    ...
                                    console.error("Auto add to krouter9 failed:", e);
                                }
                            }
                        }
