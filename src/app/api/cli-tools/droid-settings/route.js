...
// Check if settings has KRouter9 customModels
const hasKRouter9Config = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:KRouter9"));
};
...
// POST - Update KRouter9 customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing KRouter9 configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:KRouter9"));
...
// DELETE - Remove KRouter9 customModels only (keep other settings)
