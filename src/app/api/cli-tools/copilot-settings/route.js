...
const hasKRouter9Config = (config) => {
  ...
  return config.some((entry) => entry.name === "KRouter9");
};

const getKRouter9Entry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "KRouter9") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getKRouter9Entry(config);
...
// POST - Apply KRouter9 config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing KRouter9 entry or append
    const idx = config.findIndex((e) => e.name === "KRouter9");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove KRouter9 entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "KRouter9");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "KRouter9 removed from Copilot config",
    });
  } catch (error) {
