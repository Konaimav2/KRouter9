...
// Check if settings has KRouter9 config
const hasKRouter9Config = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["krouter9"];
};

// Read per-agent models.json and return current model id (without "krouter9/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["krouter9"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasKRouter9: hasKRouter9Config(settings),
...
// POST - Update KRouter9 settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `krouter9/${model}`;

    // Remove all old krouter9/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("krouter9/"))
    ...
    // Add fresh krouter9 models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`krouter9/${m}`] = {};
    });

    // Remove old krouter9 model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("krouter9/")) {
    ...
    // Update models.providers.krouter9 with all models
    settings.models.providers["krouter9"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `krouter9/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove KRouter9 settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove KRouter9 from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["krouter9"];
    ...
    // Remove krouter9 models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("krouter9/"));
    ...
    // Reset agents.defaults.model.primary if it uses krouter9
    if (settings.agents?.defaults?.model?.primary?.startsWith("krouter9/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "KRouter9 settings removed successfully",
    });
  } catch (error) {
