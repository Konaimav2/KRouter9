...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-krouter9.env");
};
...
const hasKRouter9Config = (config) => {
  ...
  if (providers["krouter9"]) return true;
...
export async function GET() {
  ...
  const hasKRouter9 = hasKRouter9Config(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_KROUTER9_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile krouter9",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["krouter9"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_KROUTER9_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "krouter9 configuration removed from jcode",
    });
  } catch (error) {
