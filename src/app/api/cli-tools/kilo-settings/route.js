...
const hasKRouter9Config = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["krouter9"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("krouter9");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasKRouter9: hasKRouter9Config(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "KRouter9", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "KRouter9 settings removed from Kilo Code" });
  } catch (error) {
