...
const hasKRouter9Config = (globalState) => {
  if (!globalState) return false;
  const isOpenAi =
    globalState.actModeApiProvider === "openai" || globalState.planModeApiProvider === "openai";
  const baseUrl = globalState.openAiBaseUrl || "";
  return isOpenAi && (baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("krouter9"));
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasKRouter9: hasKRouter9Config(globalState),
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "KRouter9 settings removed from Cline" });
  } catch (error) {
