...
const hasKRouter9Config = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["krouter9"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["krouter9"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasKRouter9: hasKRouter9Config(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("krouter9/") ? config.model.replace(/^krouter9\//, "") : null,
...
// POST - Apply KRouter9 as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_krouter9";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing krouter9 provider entry and its models
    const existingProvider = config.provider["krouter9"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["krouter9"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `krouter9/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `krouter9/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("krouter9/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove KRouter9 provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["krouter9"]?.models) {
      delete config.provider["krouter9"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["krouter9"].models).length === 0) {
        delete config.provider["krouter9"];
        if (config.model?.startsWith("krouter9/")) delete config.model;
      } else if (config.model === `krouter9/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["krouter9"].models);
        config.model = `krouter9/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire krouter9 provider
      if (config.provider) delete config.provider["krouter9"];
      if (config.model?.startsWith("krouter9/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("krouter9/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "KRouter9 settings removed from OpenCode",
    });
  } catch (error) {
