...
// Settings
export {
  getSettings, getCachedSettings, invalidateSettingsCache, updateSettings, isCloudEnabled, getCloudUrl, exportSettings,
...
// KRouter9 automation repos (codebuddy accounts/jobs + ammail OTPs)
export {
...
export async function exportDb() {
  ...
  const out = {
    ...
    apiKeys: db.all(`SELECT * FROM apiKeys`).map((r) => ({ id: r.id, key: r.key, name: r.name, machineId: r.machineId, isActive: r.isActive === 1, createdAt: r.createdAt, rpmLimit: r.rpmLimit ?? 0, tpmLimit: r.tpmLimit ?? 0, modelPolicy: r.modelPolicy || "off", allowedModels: r.allowedModels ?? null, blockedModels: r.blockedModels ?? null })),
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    for (const k of payload.apiKeys || []) {
      db.run(
        `INSERT OR REPLACE INTO apiKeys(id, key, name, machineId, isActive, createdAt, rpmLimit, tpmLimit, modelPolicy, allowedModels, blockedModels) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [k.id, k.key, k.name || null, k.machineId || null, k.isActive === false ? 0 : 1, k.createdAt || new Date().toISOString(), k.rpmLimit || 0, k.tpmLimit || 0, k.modelPolicy || "off", k.allowedModels ?? null, k.blockedModels ?? null]
      );
    }
