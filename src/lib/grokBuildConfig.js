...
const previousDefaultRegExp = /^# krouter9-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# krouter9-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via KRouter9 gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# krouter9-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# krouter9-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
