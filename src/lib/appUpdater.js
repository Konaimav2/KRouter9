...
// Collect PIDs of all krouter9-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from krouter9 install dir or wrapper cli.js
        const isAppProcess = lower.includes("krouter9") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("krouter9") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "krouter9");
  }
...
// Resolve npx/krouter9 binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx krouter9` — works regardless of global bin path changes after npm i -g
